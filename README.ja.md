...
- `REDMINE_READ_ONLY`: `true`（または `1`）を指定すると読み取り専用モードで起動します。このモードでは、作成・更新・削除（アーカイブ／アーカイブ解除、ウォッチャーの追加／削除を含む）といった書き込み系ツールがツール一覧から非表示になり、呼び出されても拒否されます。利用できるのは参照系ツール（list / get / show）のみです。デフォルトは `false` です。
